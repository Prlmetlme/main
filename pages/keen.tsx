...
const Keen: NextPage = () => {
  ...
  const [currentSlide, setCurrentSlide] = useState(0) // TODO - change back to 0
  const [animations, setAnimations] = useState(400)
  const [transitions, setTransitions] = useState(true)

  const techs: string[] = ["Bootstrap", "React.js", "Next.js", "Django", "Vanilla Javascript", "Python",
                            "MySQL", "Tailwind", "HTML5", "CSS", "SCSS", "Git", "Node.js", "REST", "Typescript", 
                          ]
...
// Copies contact info to clipboard (Honestly, this is self-explanatory)
  const copyChildToClipboard = (e: any) => {
    const element = e.target as HTMLElement
    const text: string = e.target.lastChild.innerText


    // navigator.clipboard.writeText(text)
    element.style.color = '#3bd90b'
    console.log(parent)
  }
        ...
        <div className='keen-slider__slide bg-zinc-900 h-screen text-slate-300 flex flex-col'>
          ...
          <div className='w-full h-fit text-black flex pt-6 pb-2 px-4 items-center top-bar'>
            ...
            <div className="resume growing">
              ...
              <label id='animations-label' className='text-white animate-pulse' htmlFor="animations">Animations</label>

          </div>
          <div className='flex h-5/6 px-5 justify-center'>
              <h1 className='absolute left-0 right-0 mx-auto max-w-max text-4xl text-yellow-300 pop-in2'>Full stack developer</h1>
...
{/* Third slide */}
        <div className='keen-slider__slide bg-red-500 h-screen flex justify-center text-xl p-6'>
          ...
          <div className='self-center'>
            <div>
              <div onClick={copyChildToClipboard} className='hover:scale-110 transition'>Email: <span className='text-blue-900 underline hover:text-white transition-colors'> BrandonClarke2020@Gmail.com</span></div>
            </div>
